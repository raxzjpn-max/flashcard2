...
const completeVerbMatch = html.match(/const completeVerbVocabulary=(\[[\s\S]*?\]);\r?\nconst completeVerbKnown/);
if (!completeVerbMatch) throw new Error("Data kata kerja lengkap tidak ditemukan");
const completeVerbs = new Function(`return ${completeVerbMatch[1]}`)();
const completeVerbKeys = new Set(sets.flat().map(word => `${norm(word.k)}|${norm(word.h)}`));
const addedCompleteVerbs = completeVerbs.filter(word => !completeVerbKeys.has(`${norm(word.k)}|${norm(word.h)}`)).map(word => ({...word,level:"N4-N3",kind:"kotoba"}));
for (let offset = 0; offset < addedCompleteVerbs.length; offset += 25) sets.push(addedCompleteVerbs.slice(offset, offset + 25));
const latestMatch = html.match(/const latestPdfAdditions=(\[[\s\S]*?\]);\r?\nconst completeVerbVocabulary/);
if (!latestMatch) throw new Error("Data PDF terbaru tidak ditemukan");
const latestWords = new Function(`return ${latestMatch[1]}`)();
